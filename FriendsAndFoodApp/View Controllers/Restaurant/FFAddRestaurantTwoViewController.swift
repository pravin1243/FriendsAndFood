//
//  FFAddRestaurantTwoViewController.swift
//  FriendsAndFoodApp
//
...
import UIKit
import DropDown
class FFAddRestaurantTwoViewController: FFAddRestaurantBaseController, UITextFieldDelegate {
    ...
    func validateStepOneFields() -> Bool{ // validating fields in step 1 of add recipe
        ...
        if let establisment = estNameTextField.text, !establisment.isEmpty {
            ...
            restaurantRequest?.name = establisment
        ...
        if let country = countryTextField.text, !country.isEmpty {
            ...
            restaurantRequest?.countryId = "\(selectedCountry?.idInt ?? 0)"
            restaurantRequest?.countryName = country
        ...
        if let city = cityTextField.text, !city.isEmpty {
            ...
            restaurantRequest?.cityId = "\(selectedCity?.idInt ?? 0)"
            restaurantRequest?.cityName = city
        ...
        if let postal = postalTextField.text, !postal.isEmpty {
            ...
            restaurantRequest?.postalCode = postal
        ...
        if let address = addressAdditionTextField.text, !address.isEmpty {
            ...
            restaurantRequest?.address = address
            
        }
//        else {
//            FFBaseClass.sharedInstance.showAlert(mesage: "Enter address", view: self)
//            return false
//        }
        ...
        if let restEmail = restEmailTextField.text, !restEmail.isEmpty {
            ...
            FFBaseClass.sharedInstance.showAlert(mesage: "\(StringConstants.restaurantstore.Enterrestaurantemail)", view: self)
            return false
        }
    ...
    @IBAction func nextBtnTapped(_ sender : Any){
        if validateStepOneFields(){
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "FFAddRestaurantThreeVC") as! FFAddRestaurantThreeViewController
